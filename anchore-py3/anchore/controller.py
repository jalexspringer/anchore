...
class Controller(object):
    ...
    def merge_policies(self, poldst, polsrc):
        ...
        for sk in list(polsrc.keys()):
            if sk not in polret:
                polret[sk] = polsrc[sk]

            for smk in list(polsrc[sk].keys()):
    ...
    def execute_gates(self, image, refresh=True):
        ...
        paramlist = list()
        for p in list(policies.keys()):
            for t in list(policies[p].keys()):
