...
def feed_group_data_exists(feed, group, datafile):
    feedmeta = load_anchore_feedmeta()
    if feed in list(feedmeta.keys()) and group in list(feedmeta[feed]['groups'].keys()) and 'datafiles' in feedmeta[feed]['groups'][
    ...
    try:
        for feed in list(feedmeta.keys()):
            if feedmeta[feed]['subscribed']:
                ...
                groups = list(feedmeta[feed]['groups'].keys())
                if groups:
                    for group in groups:
                        sincets = 0
                        group_meta = {}
...
def load_anchore_feed_groups_list(feed):
    ...
    feedmeta = load_anchore_feedmeta()
    if feedmeta:
        if feed in list(feedmeta.keys()):
            ret = list(feedmeta[feed]['groups'].values())
...
def load_anchore_feed_group_datameta(feed, group):
    ...
    feedmeta = load_anchore_feedmeta()
    if feedmeta:
        if feed in list(feedmeta.keys()) and group in list(feedmeta[feed]['groups'].keys()):
    ...
    if not feedmeta:
        ...
        if datameta and 'datafiles' in datameta:
            ...
            for datafile in sorted(datameta['datafiles']):
                ...
                if ensure_unique:
                    for el in thelist:
                        if isinstance(el, dict) and len(list(el.keys())) == 1:
                            elkey = list(el.keys())[0]
            ...
            if ret['success'] and ensure_unique:
                ret['data'] = list(unique_hash.values())
    ...
    if feed == 'imagedata':
        ...
        if d and d['success']:
            for record in d['data']:
                if 'redirecturl' in list(record.keys()):
                    for tag in list(record['redirecturl'].keys()):
