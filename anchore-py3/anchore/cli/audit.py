...
def generate_reports(imagelist, showall=True, showdetails=True):
    ...
    for imageId in imagelist:
        ...
        if showdetails:
            try:
                pnum = str(len(list(anchore_utils.load_analysis_output(imageId, 'package_list', 'pkgs.all').keys())))
            except:
                pnum = "N/A"
            try:
                fnum = str(len(list(anchore_utils.load_analysis_output(imageId, 'file_list', 'files.all').keys())))
            except:
                fnum = "N/A"
            try:
                snum = str(len(list(anchore_utils.load_analysis_output(imageId, 'file_suids', 'files.suids').keys())))
            except:
                snum = "N/A"
