...
class Navigator(object):
    ...
    def generate_reports(self):
        ...
        for imageId in self.images:
            ...
            try:
                pnum = str(len(list(anchore_utils.load_analysis_output(image.meta['imageId'], 'package_list', 'pkgs.all').keys())))
            except:
                pnum = "N/A"
            try:
                fnum = str(len(list(anchore_utils.load_analysis_output(image.meta['imageId'], 'file_list', 'files.all').keys())))
            except:
                fnum = "N/A"
            try:
                snum = str(len(list(anchore_utils.load_analysis_output(image.meta['imageId'], 'file_suids', 'files.suids').keys())))
            except:
                fnum = "N/A"
