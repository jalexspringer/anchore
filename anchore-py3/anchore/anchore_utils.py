    ...
    for v in cve_data:
        ...
        if 'FixedIn' in vuln:
            for fixes in vuln['FixedIn']:
                ...
                if vpkg in pkgmap:
                    for ivers in list(pkgmap[vpkg]['versions'].keys()):
...
def cve_scanimage(cve_data, image):
    ...
    for p in list(pkgsplussource.keys()):
        if p not in all_packages:
